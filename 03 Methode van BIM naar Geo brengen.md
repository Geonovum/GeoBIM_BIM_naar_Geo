...
De IfcEnvelopeExtractor is een open source C++ applicatie die BIM modellen in de IFC encoding kan omzetten naar Wavefront OBJ, STEP en CityGML CityJSON. Deze omzetting is niet een 1:1 omzetting naar een ander bestandstype. De applicatie zet de geometrie van het input bestand om naar GIS representaties die zich houden aan de GIS syntax. Het volgt hierbij het LoD framework van Biljecki. Aanvullend zijn er een aantal experimentele LoD en een 1:1 conversie beschikbaar. In totaal support de tool 17 verschillende LoD, zie appendix ... voor meer informatie.

De applicatie is verkrijgbaar via de [GitHub pagina](https://github.com/tudelft3d/IFC_BuildingEnvExtractor). Op deze pagina zijn de source code en gecompilde executables beschikbaar. De gecompilde executables zijn beschikbaar voor zowel Windows als Linux (Ubuntu). Extra informatie over de methodes die gebruikt zijn in de code om de abstractie uit te voeren kan worden gevonden in het [technische report](https://research.tudelft.nl/en/publications/bim2geo-converter/) van versie 0.2.6. Dit report is gebaseerd op een verouderde versie van de code, versie 0.3.x is de huidige versie, echter is veel van de dieper liggende logica gelijk of vergelijkbaar gebleven.
