...
Een bestand dat het GIS data model volgt kan alleen worden gecreëerd door een vorm van shell extractie. Bij shell extractie wordt een schil model gemaakt van een (of meer) BIM model(len). Dit is een lossy process, zowel voor de geometrie als de attributen gaat data gaat verloren. Volumetrische externe objecten zoals wanden en daken worden oppervlaktes die deel zijn van een volumetrische schil. Interne elementen kunnen volledig vervallen. Dit proces is dus ook niet omkeerbaar.
