...
## CityGML en CityJSON
Het is ook mogelijk om een 1 op 1 vertaling naar de CityGML (JSON encoding) te maken. Voor een BIM gebouw-model kan een directe mapping worden gemaakt, waarbij IFC entiteiten worden vertaald naar de entiteiten "Building", "Buildingpart" en "Buildingroom" zoals deze in het CityGML/CityJSON schema bestaan. Dit geeft meer betekenis aan de dataset dan de directe GeoJSON vertaling. De IFC-attributen vertaalt men direct naar de overeenkomstige CityJSON "attributes".
Veel geo-viewers kunnen een GeoJSON, CityJSON of CityGML bestand openen. Ondanks dat in CityJSON/CityGML enige betekenis van het oorspronkelijke model is geduid, geldt voor al deze modellen dat analyses maar beperkt kunnen worden toegepast. Analysesoftware verwacht vaak een specificiek datamodel. Doordat het 1-op-1 vertaalde model hier meestal niet aan voldoet kan dit resulteren in trage, onbetrouwbare of niet functionele analyses.

Bijvoorbeeld: Een zonlichtsimulatie test alle vlakken die mogelijk zonlicht ontvangen. Bij een shell extractie zal deze analyse snel gedaan kunnen worden, zie [shell extractie](#shell-extractie), omdat de buitenkant van een bouwwerk bekend is. Bij een 1 op 1 vertaling is dat niet het geval en zal deze analyse op alle oppervlaktes van het model gedaan worden. Dit resulteert er in dat deze simulatie voor zo'n model erg traag is of niet werkt.  
...
Een beperkte remedie voor de bestandsgrootte is een gefilterde 1:1 mapping. Hierbij worden alleen de objecten die deel uitmaken van de buitengevel van een gebouw naar de GIS encoding omgezet. Dit kan op een aantal verschillende manieren geïmplementeerd worden. De meest voorkomende methode maakt gebruik van een object attribuut (bijvoorbeeld [isExternal](https://ifc43-docs.standards.buildingsmart.org/IFC/RELEASE/IFC4x3/HTML/property/IsExternal.htm) in IFC). Dit is een gemakkelijke en snelle filtering en zal tijdens de conversie weinig problemen opleveren. Echter deze methode vertrouwt geheel op het correcte gebruik van dit attribuut door de makers van het model. Het correcte gebruik van attributen kan niet altijd worden gegarandeerd (mensen maken fouten, zijn onbekend met het attribuut of het belang ervan) en mogelijk moet daarom het resulterende model gecorrigeerd worden.
